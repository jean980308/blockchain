...
Original file is located at
    https://colab.research.google.com/drive/18xQ8ukpXW4jazyJzQH2FRuvMFnobmhwY
...
class BlockChain:
    def __init__(self):
        ...
        # For P2P connection
        self.socket_host = "140.118.122.186" # IP位置 Default 為本地端 
        self.socket_port = int(sys.argv[1])  # 當唯有一個節點
        self.node_address = {f"{self.socket_host}:{self.socket_port}"}
        self.connection_nodes = {}

        if len(sys.argv) == 3:  # 多於一個節點 (限制：一個command line最多增加一個)
    ...
    def create_genesis_block(self):
        ...
        new_block = Block('Hello World!', self.difficulty, 'Credit : lkm543', self.miner_rewards)
    ...
    def mine_block(self, miner):
        ...
        while new_block.hash[0: self.difficulty] != '0' * self.difficulty:
            new_block.nonce += 1
            new_block.hash = self.get_hash(new_block, new_block.nonce)
            if self.receive_verified_block:  # already be mined, change to other block 
        ...
        if len(self.chain) % self.adjust_difficulty_blocks != 1:
            ...
            if average_time_consumed > self.block_time:
                ...
                if self.difficulty < 5:
                    ...
                    print("Due to time constraint, difficulity remains at 5")
    ...
    # mine to new block, broadcast to other nodes
    def broadcast_message_to_nodes(self, request, data=None):
        ...
        for node_address in self.node_address:
            if node_address != address_concat:
                ...
                client.close()
    # recieve new block from other node
    def receive_broadcast_block(self, block_data):
        last_block = self.chain[-1]
        # Check the hash of received block
        if block_data.previous_hash != last_block.hash:  # check fail
